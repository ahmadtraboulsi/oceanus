/*
 Generator of WSPR channel symbols and complex envelope.

 File name: wsprsim.c (first committed to wsjtx June 13, 2015)
 Edited and commented by: Michel Barbeau, Carleton University
...
// Generator of complex envelope
// Inputs
//    f0 =
//    t0 =
//    amp = amplitude
//    symbols = 162 channel symbols in [0,1,2,3]
// Outputs
//    isig = in-phase of complex image (an array of size 162*256 samples)
//    qsig = quadrature of complex image (an array of size 162*256 samples)
                      ...
                      double* isig, double* qsig)
{
    ...
    //    - symbol:modulation 0:-3Hz, 1:-1Hz, 2:1Hz 3:3 Hz
    //    - Bandwidth 6 Hertz
    int i, j, ii, idelay;
    double phi=0.0, twopidt, df, dt, dphi;
    twopidt=8.0*atan(1.0)/375.0; // 2*pi/375
    df=375.0/256.0;
    dt=1/375.0;
    idelay=t0/dt;

    // for every channel symbol
    for (i=0; i<162; i++) {
        // as function of symbol, determine angular change per symbol
        dphi=twopidt*(f0 + ( (double)symbols[i]-1.5)*df );
        // for every sample in channel symbol
        for ( j=0; j<256; j++ ) {
            // determine position of sample in arrays
            ii=idelay+256*i+j;
